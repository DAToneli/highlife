...
public class HighLife implements Runnable {
  ...
  public HighLife(int soma, int[][] grid, int at) {
    this.soma = soma;
    this.grid = grid;
    this.at = at;
  }
  ...
  public static void main(String[] args) {
    ...
    HighLife[] highlifes = new HighLife[NT];
    Thread[] threads = new Thread[NT];
    
    System.out.println("**HighLife");
    ...
    for(h = 0; h < MAX; h++) {
      ...
      for(i = 0; i < NT; i++) {
        ...
        highlifes[i] = new HighLife(soma, grid, i);
        threads[i] = new Thread(highlifes[i],nome);
        threads[i].start();
      }
      ...
      for(i = 0; i < NT; i++) {
        soma += highlifes[i].getSoma();
        temp = highlifes[i].getNewgrid();
        for(j = 0; j < M; j++) {
          for(k = 0; k < N; k++) {
